...
export const PAYMENT_STRINGS = {
  ...
  syncNothingBody: 'No pending offline payments to sync.',
  ...
  topUpAmountLabel: 'Choose amount in USD',
  topUpCustomChip: 'Custom',
  topUpCustomLabel: 'Custom amount (USD)',
  topUpFxPreview: (usd: string, kgs: string, rate: string) =>
    `${usd} USD → ${kgs} KGS at ${rate}`,
  topUpFxLoading: 'Loading NBKR exchange rate...',
  topUpFxSource: (source: string, date: string) => `${source} rate date: ${date}`,
  topUpFxFallback: 'Rate unavailable, demo fallback rate used.',
  topUpFxRefresh: 'Refresh rate',
  topUpFxLock: 'Rate is locked for 30 seconds after confirm',
  topUpFxLockHint: 'Rate will lock for 30 seconds after confirm.',
  topUpFxLockedRemaining: (seconds: number) => `Locked rate expires in ${seconds}s`,
  topUpStepAuthorizeCard: 'Authorizing card',
  topUpStepReceiveQr: 'Receiving local QR payment',
  topUpStepCharge: 'Charging card',
  topUpStepConvert: 'Converting USD to KGS',
  topUpStepCredit: 'Crediting wallet',
  ...
  topUpCardCvc: 'CVC',
  topUpConfirm: (amount: string) => `Top up $${amount}`,
  topUpProcessing: 'Processing top up...',
  topUpSuccessTitle: 'Top up complete',
  topUpInvalidAmount: 'Enter a valid amount.',
  topUpInvalidCard: 'Enter a valid demo card number, expiry and CVC.',
  topUpRateStillLoading: 'Rate is still loading.',
  topUpWalletLimit: 'This top-up exceeds your wallet limit.',
  topUpDemoLimit: 'Maximum top-up is $1,000 in demo mode.',
  topUpDemoCards:
    'Demo cards: Visa 4242 4242 4242 4242, Mastercard 5555 5555 5555 4444',
  topUpUseDemoVisa: 'Use demo Visa',
  topUpCardEnding: (brand: string, last4: string) => `${brand} ending in ${last4}`,
  topUpReceived: 'Received',
  topUpCharged: 'Charged',
  topUpFxRate: 'FX Rate',
  topUpCard: 'Card',
  topUpRemainingBalance: 'Remaining Spendable Balance',
  topUpLocalQrTitle: 'Local QR top-up',
  topUpLocalQrSubtitle: 'Scan this QR with a local banking app',
  topUpQrExpiresIn: (time: string) => `Expires in ${time}`,
  topUpQrExpired: 'QR expired',
  topUpGenerateNewQr: 'Generate new QR',
  topUpSimulateQrReceived: 'Simulate payment received',
  topUpCancel: 'Cancel',
  ...
  payOfflineWarningTitle: 'Reserve-backed QR',
  payOfflineWarningBody:
    'Generating the QR does not deduct money yet. After the merchant accepts it, the reserved amount moves to pending sync and cannot be cancelled.',
  payOfflineTrustBody:
